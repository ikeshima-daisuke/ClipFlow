...
- ⌨️ **`Ctrl+Shift+V`** で履歴ポップアップを表示（トレイメニューから**好きなキー組み合わせに変更可能**）
...
既定の `Ctrl+Shift+V` が他アプリと衝突する場合などは、タスクトレイアイコンを右クリック → **「ショートカットを変更...」** から好きな組み合わせに変更できます。入力欄をクリックして押したいキーの組み合わせ（修飾キー + 1キー）を押すと表示され、「保存」で反映されます。他アプリが既に使っている組み合わせは登録に失敗するので、その場合は別の組み合わせを試してください。
